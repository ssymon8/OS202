...
Model::get_coords(std::unordered_map<std::size_t, std::uint8_t> m_fire_front)
{
    std::vector<std::size_t> indices_coords;
    for (auto f: m_fire_front)
    {
        indices_coords.push_back(f.first);
    }
...
Model::get_powers(std::unordered_map<std::size_t, std::uint8_t> m_fire_front)
{
    std::vector<std::uint8_t> powers;
    for (auto f: m_fire_front)
    {
        powers.push_back(f.first);
    }
...
bool Model::update() {
    ...
    // Récupère les clés dans un vecteur
    std::vector<std::size_t> indices_coords = get_coords(m_fire_front);
    std::size_t k = 0;
    ...
    #pragma omp for
    for (ulong indice = 0; indice < indices_coords.size(); indice++) {
        // Récupération de la coordonnée lexicographique de la case en feu :
        auto v= indices_coords[indice];
    ...
    indices_coords = get_coords(m_fire_front);
